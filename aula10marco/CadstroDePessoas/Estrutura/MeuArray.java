package CadstroDePessoas.Estrutura;
...
//        }   
    ...
    /**
     * Ordena por nome, um array de objetos do Tipo Abstrato de Dados
     * Pessoa.
     */

    public static void ordenacaoInsertionSort() {
        for (int i = 1; i < indice; i++) {
            Pessoa temp = lista[i];
            int j = i;
            while ((j > 0) && (lista[j - 1].getNome().compareToIgnoreCase(temp.getNome()) > 0)) {
                lista[j] = lista[j - 1];
                j--;
            }
            lista[j] = temp;
